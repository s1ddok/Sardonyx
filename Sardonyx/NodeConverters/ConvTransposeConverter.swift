...
class ConvTranspose2DConverter: NodeConverter, ActivationConverterInjectable {
    ...
    func contributeProperties(using context: GenerationContext) {
        context.sourceBuilder.add(line: "var layer_\(self.node.name): TransposedConv2D<Float>")
    }
    
    func contributeInit(using context: GenerationContext) {
        context.sourceBuilder.add(line: "let weight_\(self.node.name) = Tensor<Float>(shape: [\(self.kernel.height), \(self.kernel.width), \(self.outputChannels), \(self.inputChannels)], scalars: UnsafeBufferPointer<Float>(start: data.advanced(by: \(self.weightOffset)).assumingMemoryBound(to: Float.self), count: \(self.kernel.width * self.kernel.height * self.inputChannels * self.outputChannels)), on: device)")
        if self.hasBias {
        ...
        context.sourceBuilder.add(line: "self.layer_\(self.node.name) = TransposedConv2D<Float>(filter: weight_\(self.node.name), bias: bias_\(self.node.name), activation: \(activationClosure), strides: (\(self.strides.width), \(self.strides.height)), padding: .\(self.padding))")
    }
