class LeakyRELUMetalConverter: NodeConverter, FusableMetalNeuron {
    ...
    var descriptor: String { "let neuronDescriptor_\(self.node.name) = MPSNNNeuronDescriptor.cnnNeuronDescriptor(with: .reLU, a: \(self.alpha))"}
